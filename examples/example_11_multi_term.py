...
def build_pyomo_model(data):
    ...
    # PATTERN: Sum of two independent variables on the LHS
    # Requires AST BinOp(Add) recursive parsing
    def cap_rule(m, p):
...
def build_vectorized_model(data):
    ...
    df_std = pd.DataFrame(index=idx_ps)
    df_std = df_std.gppd.add_vars(m, name="x_std")
    
    idx_pe = pd.MultiIndex.from_product([data['Plants'], data['ExpLines']], names=['p', 'e'])
    df_exp = pd.DataFrame(index=idx_pe)
    df_exp = df_exp.gppd.add_vars(m, name="x_exp")
    ...
    # --- Constraint 1: Multi-Term Addition ---
    ...
    # --- Constraint 2: Multi-Term Objective ---
